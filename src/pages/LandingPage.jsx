  ...
  return (
    <div className="noise min-h-screen overflow-hidden bg-canvas">
      ...
      <main>
        ...
        {content.faqs?.length > 0 && <section className="pb-28"><div className="container-shell max-w-4xl"><div className="text-center"><span className="eyebrow">FAQs</span></div><div className="mt-10 space-y-3">{content.faqs.map((faq) => <details key={faq.content_key} className="panel p-5"><summary className="cursor-pointer font-semibold">{faq.content.question}</summary><p className="mt-3 text-sm leading-6 text-slate-400">{faq.content.answer}</p></details>)}</div></div></section>}
      </main>
      <Footer />
    </div>
  );
